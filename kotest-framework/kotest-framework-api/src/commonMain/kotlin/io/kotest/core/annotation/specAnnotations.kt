 ...
 * to define if a test will be instantiated or executed.
 * 
 * [SpecEnabaledIf] must contain a no-arg constructor as it will be used via reflection.
 */
@Target(AnnotationTarget.CLASS)
@Retention(AnnotationRetention.RUNTIME)
annotation class EnabledIf(val enabledIf: KClass<out SpecEnabledIf>)

abstract class SpecEnabledIf {
    abstract fun enabled(specKlass: KClass<out Spec>): Boolean
}